...
export const layerManager = {
  ...
  /**
   * 安全地设置颜色透明度
   * @param {*} color - 颜色值（可能是Cesium.Color、字符串、数组等）
   * @param {number} opacity - 透明度 (0-1)
   * @returns {Cesium.Color} 带透明度的Cesium颜色对象
   */
  _setColorOpacity(color, opacity) {
    try {
      // 如果颜色为空或未定义，返回默认颜色
      if (!color) {
        console.warn("颜色值为空，使用默认蓝色");
        return new Cesium.Color(0, 0, 1, opacity); // 默认蓝色
      }

      // 如果已经是Cesium.Color对象且有withAlpha方法
      if (color && typeof color.withAlpha === "function") {
        // 保持原始颜色的RGB值，只改变透明度
        return new Cesium.Color(color.red, color.green, color.blue, opacity);
      }
      ...
      // 如果是数组 [r, g, b] 或 [r, g, b, a]
      if (Array.isArray(color)) {
        if (color.length >= 3) {
          return new Cesium.Color(color[0], color[1], color[2], opacity);
        }
      }

      // 如果是对象 {red, green, blue} 或 {r, g, b}
      if (typeof color === "object" && color !== null) {
        const r = color.red || color.r || 0;
        const g = color.green || color.g || 0;
        const b = color.blue || color.b || 1;
        return new Cesium.Color(r, g, b, opacity);
      }

      // 默认返回蓝色带透明度
      console.warn("无法识别的颜色格式，使用默认蓝色");
      return new Cesium.Color(0, 0, 1, opacity);
    } catch (error) {
      console.warn("设置颜色透明度时出错:", error);
      return new Cesium.Color(0, 0, 1, opacity); // 默认蓝色
    }
  },
    ...
    layer.entities.forEach((entity) => {
      try {
        if (entity.point && entity.point.color) {
          entity.point.color = this._setColorOpacity(
            entity.point.color,
            opacity
          );
        }
        if (entity.billboard && entity.billboard.color) {
          entity.billboard.color = this._setColorOpacity(
        ...
        if (
          ...
          entity.polyline.material.color = this._setColorOpacity(
            entity.polyline.material.color,
            opacity
          );
        }
        if (
          ...
          entity.polygon.material.color = this._setColorOpacity(
            entity.polygon.material.color,
            opacity
          );
        }
      } catch (error) {
        console.warn(`设置实体透明度时出错:`, error);
      }
    });
