<template>
  <div class="popBox">
    <!-- <div style="display: flex" @click="abc">
      <div
        style="
          width: 30px;
          height: 30px;
      ...
      <div
        ...
        "
      >
        <div style="color: white; padding-top: 5px; font-size: 14px">abc</div>
      </div>
    </div> -->
    ...
    <div style="
      ...
      <div style="
          ...
          padding-bottom: 8px;
        ">
        <img style="width: 16px; height: 16px; padding-top: 5px" src="./image/标题.svg" />
        服务列表
      </div>
        ...
        <div v-for="(item, index) in allListArr">
          <div style="
            ...
            <div style="
                ...
                padding-right: 8px;
              ">
              <el-checkbox style="padding-left: 10px" v-model="item.checkAll" :indeterminate="item.isIndeterminate"
                @change="handleCheckAllChange($event, item)">
                {{ item.name }}
              </el-checkbox>
              <div @click="changeExpend(item)">
                <img v-if="!item.expanded" style="cursor: pointer; width: 16px; height: 16px" src="./image/下.svg" />
                <img v-if="item.expanded" style="cursor: pointer; width: 16px; height: 16px" src="./image/上.svg" />
              </div>
            </div>

            <div style="padding-left: 32px" v-show="item.expanded" v-for="layer in item.data">
              <div v-show="layer.showFlag">
                <el-checkbox @change="handleCheckChange($event, layer, item)" v-model="layer.checked" :key="layer.id"
...
import { computed, defineProps, watch, ref, nextTick } from "vue";
let show = ref(false);
let urlImg = ref();
const setShow = (value) => {
  show.value = value;
...
// const ffCesium = ref(props.ffCesium);
const ffCesium = computed(() => {
  return props.ffCesium;
});
// 优化：只在必要时监听，避免不必要的重新渲染
watch(
  () => props.ffCesium,
  (newValue, oldValue) => {
    if (newValue && newValue !== oldValue) {
      console.log("ffCesium updated", newValue);
    }
  },
  { immediate: true, deep: false } // 避免深度监听
);
...
// 图层操作防抖
const debouncedLayerOperation = debounce((operation, ...args) => {
  operation(...args);
}, 300);
...
let tangyuanDLG = ref([]); //
// 图层管理all
const allLayer = (layer: string, item: any) => {
  if (item.type == "basicMapServices") {
    addLayer(layer.type);
    ...
    let htmlCan = `
    <div style="display: flex" onclick='showInfo(${JSON.stringify(layerData)})'>
      <div
        style="
          width: 30px;
          height: 30px;
      ...
      <div
        style="
          ...
          background-size: 100% 100%;
        "
      > <div style="color: white; padding-top: 5px; font-size: 14px">${layer.name}</div></div>
    </div>`;
...
// 优化：移除基础地图服务 - 使用更高效的查找和删除
const removeLayer = (type: string) => {
  const indexToRemove = mapLayerArr.value.findIndex((lay) => lay.type === type);
  if (indexToRemove !== -1) {
    const item = mapLayerArr.value[indexToRemove];
    mapLayerArr.value.splice(indexToRemove, 1);
    pushedLayers.delete(type);
    console.log("Removing layer:", item);
    ffCesium.value.removeMapLayer(item);
  }
};
