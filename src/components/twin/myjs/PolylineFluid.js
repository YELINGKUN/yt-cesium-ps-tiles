import PolylineFlow from "../libs/polylineFlow.js";
...
class PolylineFluid {
  ...
  heightMonitor() {
    ...
    this.polylineFluidHandler.setInputAction((wheelment) => {
      ...
      if (height < 50000) {
          ...
          if (option.times != oldTimes) {
            ...
            if (option.type == "branch") {
              let newPolygonFlowObj = new PolylineFlow(
                ...
                branchImage[option.times],
                option.time
              );
              the.polylineFluidEntityArr[i].polyline.material = newPolygonFlowObj;
            } else {
              let newPolygonFlowObj = new PolylineFlow(
                Cesium.Color.fromCssColorString(option.color),
                cannalImage[option.times],
                option.time
              );
              the.polylineFluidEntityArr[i].polyline.material = newPolygonFlowObj;
            }
        ...
        console.log("the.polylineFluidEntityArr", the.polylineFluidEntityArr);
        for (let i = 0; i < the.polylineFluidEntityArr.length; i++) {
          if (the.polylineFluidEntityArr[i].option.type == "branch") {
            the.polylineFluidEntityArr[i].show = false;
          }
        }
      }
  ...
  addPolylineFluid(lnglatArr, option) {
    ...
    this.cameraHeight = heightTemp;

    /**
    option.width = new Cesium.CallbackProperty(function (time, result) {
      result = option.widthCoefficient + 100000 / the.cameraHeight; // 这里假设宽度与距离成反比，距离越大，宽度越小
      return result;
    }, false);
    if (option.type == "branch") {
      option.img = branchImage[option.times];
    } else {
      option.img = cannalImage[option.times];
    }
  ...
  async copyImagesTime(base64Image, num) {
