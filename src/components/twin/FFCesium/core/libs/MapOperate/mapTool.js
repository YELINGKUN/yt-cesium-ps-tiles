...
export const mapTool = {
  ...
  /**
   * 添加天空盒。
   * @param {Object} option - 天空盒的配置选项，包括六个面的纹理路径。
   *  option={px:'',nx:'',py:'',ny:'',pz:'',nz:''}
   */
  addSkyBox(option) {
  ...
  /**
   * 世界坐标(三维坐标转换)转经纬度
   * @param {Cesium.Cartesian3} cartesian3 - 三维坐标。
   * @returns {Object} 包含经纬度和高度的对象。
   */
  ...
  /**
   * 将经纬度高度转换为世界坐标(三维坐标转换)。
   * @param {Object} lngLatHeight - 包含经纬度和高度的对象。
   * @returns {Cesium.Cartesian3} 三维坐标。
   */
  ...
  /**
   * 打开地图信息
   * @param {Object} option - 地图信息窗口的配置选项，用于自定义样式。
   */
  openMapInfo(option) {
  ...
  /**
   * 测量线段长度。
   * @param {Function} callback - 测量完成后的回调函数，接收测量长度作为参数。
   */
  ...
  /**
   * 根据两个Cartesian3坐标计算中心点。
   * 该函数用于将两个三维坐标点转换为地理坐标，然后计算它们的地理中心点，最后将这个中心点转换回Cartesian3坐标。
   * @param {Cartesian3} from 起点的Cartesian3坐标。
   * @param {Cartesian3} to 终点的Cartesian3坐标。
   * @returns {Cartesian3} 返回计算得到的中心点的Cartesian3坐标。
   */
  ...
  /**
   * 测量面积
   * 通过点击屏幕添加点，当点的数量大于等于3时，形成一个多边形，计算并显示该多边形的面积。
   * 最后，通过回调函数返回计算的面积值。
   * @param {Function} callback - 面积计算完成后的回调函数，接收一个参数，为计算得到的面积值。
   */
  ...
  /**
   * 打开指南针工具
   * @param {Object} option - 指南针的样式选项
   * 该方法用于初始化或显示指南针工具，指南针用于指示场景中的朝向。
   * 参数`option`允许定制指南针的外观和样式。
   */
  ...
  /**
   * 关闭指南针工具
   * 该方法用于隐藏指南针，如果需要可以重新调用`openCompassTool`方法来重新显示指南针。
   */
  closeCompassTool() {
    if (this.cesiumNavigation) {
      const elements = document.getElementsByClassName("compass");
  ...
  /**
   * 打开比例尺工具
   * @param {Object} option - 比例尺的样式选项
   * 该方法用于初始化或显示比例尺工具，比例尺用于显示场景中物体的实际尺寸。
   * 参数`option`允许定制比例尺的外观和样式。
   */
  ...
  /**
   * 关闭比例尺工具
   * 该方法用于隐藏比例尺，如果需要可以重新调用`openScaleTool`方法来重新显示比例尺。
   */
  closeScaleTool() {
    if (this.cesiumNavigation) {
      const elements = document.getElementsByClassName("distance-legend");
