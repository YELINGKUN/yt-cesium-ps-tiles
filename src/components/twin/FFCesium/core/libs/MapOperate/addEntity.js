...
export const addEntity = {
  ...
  addCircleEntity(centerPoint, radius, option) {
    ...
    newOption.material = new Cesium.Color.fromCssColorString(
      newOption.color
    ).withAlpha(newOption.alpha);
    let positionTemp = Cesium.Cartesian3.fromDegrees(
      centerPoint[0],
      centerPoint[1],
      centerPoint[2]
    );
  ...
  addBillboardEntity(lngLatHeight, option) {
    ...
    if (option.heightReference) {
      positionTemp = Cesium.Cartesian3.fromDegrees(
        lngLatHeight[0],
        lngLatHeight[1]
      );
    } else {
    ...
    // newOption.scale = new Cesium.Cartesian2(5, 1.5);
