...
export const addType = {
   ...
   * @param {HTMLElement} htmlOverlay - 要添加的HTML元素
   * @param {Object} offset - 元素的偏移量
   * @returns {HTMLElement} 添加的HTML覆盖物元素
   */
  ...
  /**
   * 开启HTML覆盖物的碰撞检测
   * @param {Array} allHtmlOverlay - 所有HTML覆盖物的数组
   * @param {Object} option - 碰撞检测的选项
   */
  ...
  /**
   * 关闭div碰撞检测功能。
   * 该函数用于停止对viewer场景中camera变化的监听，并清除定时器，以避免不必要的计算和资源消耗。
   * 当div碰撞检测不再需要时，可以通过调用此函数来优化性能。
   */
  ...
  /**
   * 根据区域坐标展示地图
   * @param {Array} LngLats - 区域的经度和纬度数组
   * 该方法通过创建剪裁多边形和走廊线来在地图上显示一个特定区域。
   * 它首先配置场景以确保地形检测启用，并且背景色透明，这样可以更清晰地显示区域。
   * 然后，它创建一个剪裁多边形集合，用于剪裁地图显示区域。
   * 最后，添加一个走廊线实体，以视觉上突出显示区域边界。
   */
  showMapByAreaLngLat(LngLats) {
    ...
    //     height: -1000, // 高度
    //     width: 300, // 宽度
    //     extrudedHeight: 700, // 挤出高度,也就是通道的高度
