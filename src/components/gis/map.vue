...
onMounted(() => {
  ...
  // ffCesium.viewer.scene.globe.enableLighting = true;
  // ffCesium.viewer.scene.light = new Cesium.DirectionalLight({
  //   direction: new Cesium.Cartesian3(0.354925, -1.1290918, -0.383358),
  // });
  // ffCesium.viewer.scene.light.color = new Cesium.Color(0.8, 0.8, 0.8, 1.0);
  // ffCesium.viewer.scene.light.intensity = 200.8;
...
vec4 color = texture(colorTexture, v_textureCoordinates);
  ...
  ffCesium.viewer.scene.postProcessStages.add(
  ...
  polylineFluidObj.addPolylineFluid(smallRiverArr1, option);
  ...
  handler.setInputAction((wheelment) => {
    ...
    let num = 10000;
    if (height < num && oldHeight >= num) {
      ffCesium.viewer.scene.verticalExaggeration = 1.0;
      //重绘wall
      addWaiwei(1.0);
    } else if (height > num && oldHeight != null && oldHeight <= num) {
...
const addWaiweiArea = () => {
  ...
  // let tt = scaledPoly.geometry.coordinates[0];
  // let str = "";
  // for (let i = 0; i < tt.length; i++) {
  //   str = str + "[" + tt[i][0] + "," + tt[i][1] + "],";
  // }
  ...
  ffCesium.viewer.entities.add({
    ...
    polygon: {
      ...
      material: Cesium.Color.fromCssColorString("#4ABAE9").withAlpha(1),
    },
  });
...
const addWaiwei = async (num) => {
  ...
  for (let i = 0; i < pointList.length; i++) {
    ...
    if (pointList[i][2] > 200) {
      maximumHeightsArr.push(pointList[i][2] * num);
    } else {
      maximumHeightsArr.push(pointList[i][2] * num);
    }
  }
  ...
  if (!wallEntity) {
    wallEntity = ffCesium.viewer.entities.add({
      wall: {
    ...
    wallEntity.wall.minimumHeights = minimumHeightsArr;
    wallEntity.wall.maximumHeights = maximumHeightsArr;
  }
